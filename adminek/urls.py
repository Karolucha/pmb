...
urlpatterns = [
    ...
    url(r'^object/(?P<object_name>\w+)/(?P<pk>[0-9]+)?$', generic_views.BaseGenericView.as_view(), name='detail'),
    url(r'^actual_detail/(?P<pk>[0-9]+)?$', actual.ActualDetailView.as_view(), name='actual_detail'),
    # url(r'^cancelary/(?P<pk>[0-9]+)?$', cancelary.CancelaryNormalView.as_view(), name='cancelary_detail'),
    ...
    # url('^cancelary/(?P<pk>[0-9]+)', cancelary.CancelaryUpdateView.as_view(), name='cancelary_detail'),
]
